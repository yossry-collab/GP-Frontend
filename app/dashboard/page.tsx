...
import {
    ...
    Shield, ShieldOff, AlertCircle, Check, FileText, RefreshCw, Download, DollarSign, TrendingUp, AlertTriangle,
    Home, ChevronLeft, Menu, Gift, Coins, ShoppingBag, Clock, Eye, ChevronDown
} from 'lucide-react'
import {
...
import { usersAPI, productsAPI, adminAPI, loyaltyAPI, ordersAPI } from '@/lib/api'
import { useRouter } from 'next/navigation'
import ProtectedRoute from '@/components/ProtectedRoute'

// ─── Types ──────────────────────────────────────────────
interface UserType {
    _id: string; username: string; email: string; phonenumber?: string; role: string; createdAt?: string; isBanned?: boolean; banReason?: string
}
                ...
                <aside className={`fixed left-0 top-0 h-full z-40 bg-white dark:bg-[#16161f] border-r border-gray-200 dark:border-white/[0.06] transition-all duration-300 flex flex-col ${sidebarOpen ? 'w-[260px]' : 'w-[80px]'}`}>
                    ...
                    <div className="h-[72px] flex items-center px-5 border-b border-gray-100 dark:border-white/[0.04]">
                        <div className="flex items-center gap-2.5 cursor-pointer" onClick={() => router.push('/store')}>
                            ...
                            {sidebarOpen && <span className="text-lg font-extrabold tracking-tight text-gray-900 dark:text-white">GAME<span className="text-gradient">PLUG</span></span>}
                        </div>
                    </div>
...
function OverviewPage({ username }: { username: string }) {
    ...
    useEffect(() => {
        (async () => {
            try {
                const [res, advRes] = await Promise.all([adminAPI.getStats(), adminAPI.getAdvancedStats()])
                setStats(res.data)
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            {/* Welcome */}
            <div className="mb-8">
                <h1 className="text-[1.5rem] font-bold text-gray-900 dark:text-white">Welcome Back, BOSS</h1>
            ...
            {advancedStats && (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                    {[
                        { label: 'Avg Order Value', value: `$${(advancedStats.avgOrderValue || 0).toFixed(2)}`, icon: DollarSign, color: '#7C3AED', bg: 'bg-violet-50 dark:bg-violet-500/10' },
                        { label: 'Conversion Rate', value: `${(advancedStats.conversionRate || 0).toFixed(1)}%`, icon: TrendingUp, color: '#0891B2', bg: 'bg-cyan-50 dark:bg-cyan-500/10' },
                        { label: 'Revenue This Month', value: `$${(advancedStats.revenueThisMonth || 0).toLocaleString()}`, icon: Coins, color: '#059669', bg: 'bg-emerald-50 dark:bg-emerald-500/10' },
                        { label: 'Top Category', value: advancedStats.topCategory?.name || (typeof advancedStats.topCategory === 'string' ? advancedStats.topCategory : 'N/A'), icon: BarChart3, color: '#D97706', bg: 'bg-amber-50 dark:bg-amber-500/10' },
                    ].map((kpi, i) => (
                        <motion.div
                            key={kpi.label}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-8">
                    ...
                    >
                        <div className="p-6 pb-4">
                            ...
                            <p className="text-sm text-gray-400 mt-0.5">Best sellers by order count</p>
                        </div>
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm">
                                <thead>
                                ...
                                <tbody className="divide-y divide-gray-100 dark:divide-white/[0.04]">
                                    {(advancedStats.topProducts || []).map((p: any, i: number) => (
                                        <tr key={i} className="hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                                            ...
                                            <td className="px-6 py-3"><span className="px-2 py-0.5 rounded-md bg-primary-100 dark:bg-primary-500/15 text-primary-700 dark:text-primary-400 text-xs font-bold">{p.count}</span></td>
                                            <td className="px-6 py-3 font-semibold text-gray-700 dark:text-gray-300">${(p.revenue || 0).toLocaleString()}</td>
                                        </tr>
                                    ))}
                                    {(!advancedStats.topProducts || advancedStats.topProducts.length === 0) && (
                                        <tr><td colSpan={4} className="text-center py-8 text-gray-400 text-sm">No product data yet</td></tr>
                                    )}
                                </tbody>
                            </table>
                        </div>
                    ...
                    >
                        <div className="mb-4">
                            ...
                            <p className="text-sm text-gray-400 mt-0.5">Monthly revenue over the last 12 months</p>
                        </div>
                        <ResponsiveContainer width="100%" height={300}>
                            <AreaChart data={advancedStats.revenueByMonth || []} margin={{ top: 10, right: 10, left: -10, bottom: 5 }}>
                                <defs>
                                ...
                                <XAxis dataKey="month" axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: '#919EAB' }} dy={8} />
                                <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: '#919EAB' }} dx={-8} />
                                <Tooltip contentStyle={{ backgroundColor: 'rgba(255,255,255,0.96)', border: 'none', borderRadius: '12px', padding: '10px 14px', fontSize: '12px', boxShadow: '0 0 2px rgba(145,158,171,0.24), -20px 20px 40px -4px rgba(145,158,171,0.24)' }} />
                                <Area type="monotone" dataKey="revenue" name="Revenue" stroke="#7C3AED" strokeWidth={3} fill="url(#areaRevenue)" dot={{ fill: '#7C3AED', r: 3, strokeWidth: 0 }} activeDot={{ r: 5, stroke: '#fff', strokeWidth: 2 }} />
                            </AreaChart>
                        </ResponsiveContainer>
                    </motion.div>
                </div>
...
function UsersPage() {
    ...
    const handleBan = async (id: string) => {
        try { setBanningId(id); await usersAPI.ban(id); setMsg({ type: 'success', text: 'User banned successfully' }); fetchUsers() }
        catch (err: any) { setMsg({ type: 'error', text: err.response?.data?.message || 'Failed to ban user' }) }
        finally { setBanningId(null) }
    }

    const handleUnban = async (id: string) => {
        try { setBanningId(id); await usersAPI.unban(id); setMsg({ type: 'success', text: 'User unbanned successfully' }); fetchUsers() }
        catch (err: any) { setMsg({ type: 'error', text: err.response?.data?.message || 'Failed to unban user' }) }
        finally { setBanningId(null) }
    }
    ...
    const handleExportEmails = async () => {
        try {
            ...
            ;(data.users || []).forEach((u: any) => rows.push([u.username, u.email, u.role, u.isBanned ? 'Yes' : 'No', u.joinedAt ? new Date(u.joinedAt).toLocaleDateString() : '']))
            const csv = rows.map(r => r.join(',')).join('\n')
            const blob = new Blob([csv], { type: 'text/csv' })
            const url = URL.createObjectURL(blob)
            const a = document.createElement('a')
            a.href = url; a.download = 'mailing-list.csv'; a.click()
            URL.revokeObjectURL(url)
            setMsg({ type: 'success', text: `Exported ${(data.users || []).length} users` })
        } catch { setMsg({ type: 'error', text: 'Failed to export mailing list' }) }
    }
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            ...
            <div className="bg-white dark:bg-[#16161f] rounded-2xl border border-gray-200 dark:border-white/[0.06] overflow-hidden">
                <div className="p-5 border-b border-gray-100 dark:border-white/[0.04] flex flex-col sm:flex-row sm:items-center gap-4">
                    ...
                    <button onClick={handleExportEmails} className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-primary-600 text-white text-xs font-semibold hover:bg-primary-700 transition-all" title="Export Mailing List">
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="text-left text-[11px] text-gray-400 uppercase tracking-wider border-b border-gray-100 dark:border-white/[0.04]">
                                    <th className="px-5 py-3">User</th><th className="px-5 py-3">Email</th><th className="px-5 py-3">Phone</th><th className="px-5 py-3">Role</th><th className="px-5 py-3">Status</th><th className="px-5 py-3 text-right">Actions</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-100 dark:divide-white/[0.04]">
                                {filtered.map(u => (
                                        ...
                                        <td className="px-5 py-4">
                                            <div className="flex items-center justify-end gap-2">
                                                {editingId === u._id ? (<>
                                                    ...
                                                    {u.role !== 'admin' && (
                                                        u.isBanned ? (
                                                            <button onClick={() => handleUnban(u._id)} disabled={banningId === u._id} className="px-2 py-1 rounded-lg bg-emerald-100 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-400 text-[10px] font-bold hover:bg-emerald-200 dark:hover:bg-emerald-500/25 transition-all disabled:opacity-50" title="Unban user">
                                                                {banningId === u._id ? '...' : 'Unban'}
                                                            </button>
                                                        ) : (
                                                            <button onClick={() => handleBan(u._id)} disabled={banningId === u._id} className="px-2 py-1 rounded-lg bg-red-100 text-red-700 dark:bg-red-500/15 dark:text-red-400 text-[10px] font-bold hover:bg-red-200 dark:hover:bg-red-500/25 transition-all disabled:opacity-50" title="Ban user">
                                                                {banningId === u._id ? '...' : 'Ban'}
                                                            </button>
                                                        )
                                                    )}
...
function LoyaltyAdminPage() {
    ...
    const [rewardForm, setRewardForm] = useState({ name: '', description: '', type: 'coupon', pointsCost: 100, discountPercent: 0, discountAmount: 0, stock: -1, tierRequired: 'free', isActive: true })

    // Quest form
    const [showQuestForm, setShowQuestForm] = useState(false)
    const [editingQuest, setEditingQuest] = useState<any>(null)
    const [questForm, setQuestForm] = useState({ title: '', description: '', type: 'custom', rewardPoints: 50, featureFlag: '', isActive: true })
    ...
    const inputClass = 'w-full px-3 py-2 rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-white/[0.04] text-sm focus:ring-2 focus:ring-primary-500/30 focus:border-primary-500 outline-none transition-all'
    const btnPrimary = 'px-4 py-2 bg-primary-600 text-white text-sm font-medium rounded-xl hover:bg-primary-700 transition-all'
    const btnSecondary = 'px-4 py-2 bg-gray-100 dark:bg-white/[0.06] text-sm font-medium rounded-xl hover:bg-gray-200 dark:hover:bg-white/[0.1] transition-all'
    ...
    async function handleSeed() {
        ...
        try {
            ...
            loadAll()
        } catch { setMsg({ type: 'error', text: 'Failed to seed defaults.' }) }
        setSeeding(false)
    }

    async function handleGrant() {
        if (!grantForm.userId || grantForm.amount <= 0) return setMsg({ type: 'error', text: 'User ID and amount required.' })
        setGranting(true)
        try {
            ...
            loadAll()
        } catch { setMsg({ type: 'error', text: 'Failed to grant points.' }) }
        setGranting(false)
    }
    ...
    async function saveReward() {
        try {
            ...
            setRewardForm({ name: '', description: '', type: 'coupon', pointsCost: 100, discountPercent: 0, discountAmount: 0, stock: -1, tierRequired: 'free', isActive: true })
            loadAll()
        } catch { setMsg({ type: 'error', text: 'Failed to save reward.' }) }
    }
    ...
    function startEditReward(r: any) {
        setEditingReward(r)
        setRewardForm({ name: r.name, description: r.description || '', type: r.type, pointsCost: r.pointsCost, discountPercent: r.discountPercent || 0, discountAmount: r.discountAmount || 0, stock: r.stock, tierRequired: r.tierRequired || 'free', isActive: r.isActive })
    ...
    async function saveQuest() {
        try {
            ...
            setShowQuestForm(false); setEditingQuest(null)
            setQuestForm({ title: '', description: '', type: 'custom', rewardPoints: 50, featureFlag: '', isActive: true })
            loadAll()
        } catch { setMsg({ type: 'error', text: 'Failed to save quest.' }) }
    }
    ...
    function startEditQuest(q: any) {
        setEditingQuest(q)
        setQuestForm({ title: q.title, description: q.description || '', type: q.type, rewardPoints: q.rewardPoints, featureFlag: q.featureFlag || '', isActive: q.isActive })
    ...
    async function updateConfig(key: string, value: string) {
        try {
            ...
            loadAll()
        } catch { setMsg({ type: 'error', text: 'Failed to update config.' }) }
    }
                ...
                <div className="space-y-6">
                    ...
                    <div className="grid md:grid-cols-2 gap-4">
                        <div className={`${cardClass} p-5`}>
                            ...
                            <p className="text-xs text-gray-500 mb-3">Creates default rewards, quests, packs, memberships, and config if they don&apos;t exist.</p>
                        ...
                        <div className={`${cardClass} p-5`}>
                            ...
                            <div className="flex gap-2 flex-wrap">
                                <input placeholder="User ID" value={grantForm.userId} onChange={e => setGrantForm({ ...grantForm, userId: e.target.value })} className={`${inputClass} flex-1 min-w-[120px]`} />
                                <input type="number" placeholder="Amount" min={1} value={grantForm.amount || ''} onChange={e => setGrantForm({ ...grantForm, amount: parseInt(e.target.value) || 0 })} className={`${inputClass} w-24`} />
                                <input placeholder="Reason" value={grantForm.reason} onChange={e => setGrantForm({ ...grantForm, reason: e.target.value })} className={`${inputClass} flex-1 min-w-[100px]`} />
                                <button onClick={handleGrant} disabled={granting} className={btnPrimary}>
                                    {granting ? '...' : 'Grant'}
                                </button>
                            </div>
                        </div>
                    ...
                    {stats && (
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            {[
                                ...
                                { label: 'Points in Circulation', value: (stats.totalPointsInCirculation || 0).toLocaleString(), color: 'text-amber-600' },
                                { label: 'Total Transactions', value: stats.totalTransactions || 0, color: 'text-emerald-600' },
                                { label: 'Total Redemptions', value: stats.totalRedemptions || 0, color: 'text-purple-600' },
                            ].map((s, i) => (
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        {[
                            ...
                            <div key={i} className={`${cardClass} p-4 flex items-center gap-3`}>
                                <div className="p-2 rounded-xl bg-gray-100 dark:bg-white/[0.06]"><s.icon className="w-5 h-5 text-gray-500" /></div>
                                <div><p className="text-lg font-bold">{s.value}</p><p className="text-xs text-gray-500">{s.label}</p></div>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            {tab === 'rewards' && (
                <div className="space-y-4">
                    <div className="flex justify-between items-center">
                        ...
                        <button onClick={() => { setEditingReward(null); setRewardForm({ name: '', description: '', type: 'coupon', pointsCost: 100, discountPercent: 0, discountAmount: 0, stock: -1, tierRequired: 'free', isActive: true }); setShowRewardForm(true) }} className={btnPrimary}>
                            <span className="flex items-center gap-1.5"><Plus className="w-4 h-4" /> Add Reward</span>
                        </button>
                    </div>

                    {showRewardForm && (
                        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className={`${cardClass} p-5 space-y-3`}>
                            <h4 className="font-semibold text-sm">{editingReward ? 'Edit Reward' : 'New Reward'}</h4>
                            <div className="grid md:grid-cols-2 gap-3">
                                <input placeholder="Name" value={rewardForm.name} onChange={e => setRewardForm({ ...rewardForm, name: e.target.value })} className={inputClass} />
                                <select value={rewardForm.type} onChange={e => setRewardForm({ ...rewardForm, type: e.target.value })} className={inputClass}>
                                    <option value="coupon">Coupon</option><option value="gift_card">Gift Card</option><option value="product">Product</option><option value="points_boost">Points Boost</option>
                                </select>
                                <input type="number" placeholder="Points Cost" value={rewardForm.pointsCost} onChange={e => setRewardForm({ ...rewardForm, pointsCost: parseInt(e.target.value) || 0 })} className={inputClass} />
                                <input type="number" placeholder="Discount %" value={rewardForm.discountPercent} onChange={e => setRewardForm({ ...rewardForm, discountPercent: parseInt(e.target.value) || 0 })} className={inputClass} />
                                <input type="number" placeholder="Discount Amount" value={rewardForm.discountAmount} onChange={e => setRewardForm({ ...rewardForm, discountAmount: parseFloat(e.target.value) || 0 })} className={inputClass} />
                                <input type="number" placeholder="Stock (-1 = unlimited)" value={rewardForm.stock} onChange={e => setRewardForm({ ...rewardForm, stock: parseInt(e.target.value) })} className={inputClass} />
                                <select value={rewardForm.tierRequired} onChange={e => setRewardForm({ ...rewardForm, tierRequired: e.target.value })} className={inputClass}>
                                    <option value="free">Free tier</option><option value="silver">Silver</option><option value="gold">Gold</option>
                                </select>
                                <textarea placeholder="Description" value={rewardForm.description} onChange={e => setRewardForm({ ...rewardForm, description: e.target.value })} className={`${inputClass} min-h-[60px]`} />
                            </div>
                            <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={rewardForm.isActive} onChange={e => setRewardForm({ ...rewardForm, isActive: e.target.checked })} /> Active</label>
                            <div className="flex gap-2">
                                <button onClick={saveReward} className={btnPrimary}>{editingReward ? 'Update' : 'Create'}</button>
                                <button onClick={() => { setShowRewardForm(false); setEditingReward(null) }} className={btnSecondary}>Cancel</button>
                            </div>
                        </motion.div>
                    )}
                    ...
                    <div className={`${cardClass} overflow-hidden`}>
                        <table className="w-full text-sm">
                            <thead><tr className="border-b border-gray-100 dark:border-white/[0.06] text-left text-xs text-gray-500 uppercase">
                                <th className="px-4 py-3">Name</th><th className="px-4 py-3">Type</th><th className="px-4 py-3">Cost</th><th className="px-4 py-3">Stock</th><th className="px-4 py-3">Tier</th><th className="px-4 py-3">Active</th><th className="px-4 py-3 text-right">Actions</th>
                            </tr></thead>
                            <tbody>
                                {rewards.length === 0 ? <tr><td colSpan={7}><Empty text="No rewards yet. Seed defaults or create one." /></td></tr> : rewards.map((r: any) => (
                                    <tr key={r._id} className="border-b border-gray-50 dark:border-white/[0.03] hover:bg-gray-50/50 dark:hover:bg-white/[0.02]">
                                        <td className="px-4 py-3 font-medium">{r.name}</td>
                                        <td className="px-4 py-3"><span className="px-2 py-0.5 rounded-lg text-[10px] font-bold uppercase bg-gray-100 dark:bg-white/[0.06]">{r.type}</span></td>
                                        <td className="px-4 py-3 text-amber-600 font-bold">{r.pointsCost}</td>
                                        <td className="px-4 py-3">{r.stock === -1 ? '∞' : r.stock}</td>
                                        <td className="px-4 py-3 capitalize">{r.tierRequired || 'free'}</td>
                                        <td className="px-4 py-3">{r.isActive ? <span className="text-emerald-500">●</span> : <span className="text-gray-300">●</span>}</td>
                                        <td className="px-4 py-3 text-right flex gap-1 justify-end">
                                            <ActionBtn onClick={() => startEditReward(r)} icon={Edit3} />
                                            <ActionBtn onClick={() => deleteReward(r._id)} icon={Trash2} variant="danger" />
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
            ...
            {tab === 'quests' && (
                <div className="space-y-4">
                    <div className="flex justify-between items-center">
                        ...
                        <button onClick={() => { setEditingQuest(null); setQuestForm({ title: '', description: '', type: 'custom', rewardPoints: 50, featureFlag: '', isActive: true }); setShowQuestForm(true) }} className={btnPrimary}>
                            <span className="flex items-center gap-1.5"><Plus className="w-4 h-4" /> Add Quest</span>
                        </button>
                    </div>

                    {showQuestForm && (
                        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className={`${cardClass} p-5 space-y-3`}>
                            <h4 className="font-semibold text-sm">{editingQuest ? 'Edit Quest' : 'New Quest'}</h4>
                            <div className="grid md:grid-cols-2 gap-3">
                                <input placeholder="Title" value={questForm.title} onChange={e => setQuestForm({ ...questForm, title: e.target.value })} className={inputClass} />
                                <select value={questForm.type} onChange={e => setQuestForm({ ...questForm, type: e.target.value })} className={inputClass}>
                                    <option value="social_follow">Social Follow</option><option value="share_product">Share Product</option><option value="write_review">Write Review</option>
                                    <option value="complete_profile">Complete Profile</option><option value="first_purchase">First Purchase</option><option value="streak_login">Streak Login</option><option value="custom">Custom</option>
                                </select>
                                <input type="number" placeholder="Reward Points" value={questForm.rewardPoints} onChange={e => setQuestForm({ ...questForm, rewardPoints: parseInt(e.target.value) || 0 })} className={inputClass} />
                                <input placeholder="Feature Flag (optional)" value={questForm.featureFlag} onChange={e => setQuestForm({ ...questForm, featureFlag: e.target.value })} className={inputClass} />
                                <textarea placeholder="Description" value={questForm.description} onChange={e => setQuestForm({ ...questForm, description: e.target.value })} className={`${inputClass} md:col-span-2 min-h-[60px]`} />
                            </div>
                            <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={questForm.isActive} onChange={e => setQuestForm({ ...questForm, isActive: e.target.checked })} /> Active</label>
                            <div className="flex gap-2">
                                <button onClick={saveQuest} className={btnPrimary}>{editingQuest ? 'Update' : 'Create'}</button>
                                <button onClick={() => { setShowQuestForm(false); setEditingQuest(null) }} className={btnSecondary}>Cancel</button>
                            </div>
                        </motion.div>
                    )}
                    ...
                    <div className={`${cardClass} overflow-hidden`}>
                        <table className="w-full text-sm">
                            <thead><tr className="border-b border-gray-100 dark:border-white/[0.06] text-left text-xs text-gray-500 uppercase">
                                <th className="px-4 py-3">Title</th><th className="px-4 py-3">Type</th><th className="px-4 py-3">Points</th><th className="px-4 py-3">Flag</th><th className="px-4 py-3">Active</th><th className="px-4 py-3 text-right">Actions</th>
                            </tr></thead>
                            <tbody>
                                {quests.length === 0 ? <tr><td colSpan={6}><Empty text="No quests yet." /></td></tr> : quests.map((q: any) => (
                                    <tr key={q._id} className="border-b border-gray-50 dark:border-white/[0.03] hover:bg-gray-50/50 dark:hover:bg-white/[0.02]">
                                        <td className="px-4 py-3 font-medium">{q.title}</td>
                                        <td className="px-4 py-3"><span className="px-2 py-0.5 rounded-lg text-[10px] font-bold uppercase bg-gray-100 dark:bg-white/[0.06]">{q.type?.replace(/_/g, ' ')}</span></td>
                                        <td className="px-4 py-3 text-amber-600 font-bold">{q.rewardPoints}</td>
                                        <td className="px-4 py-3 text-xs text-gray-400">{q.featureFlag || '—'}</td>
                                        <td className="px-4 py-3">{q.isActive ? <span className="text-emerald-500">●</span> : <span className="text-gray-300">●</span>}</td>
                                        <td className="px-4 py-3 text-right flex gap-1 justify-end">
                                            <ActionBtn onClick={() => startEditQuest(q)} icon={Edit3} />
                                            <ActionBtn onClick={() => deleteQuest(q._id)} icon={Trash2} variant="danger" />
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
            ...
            {tab === 'packs' && (
                <div className="space-y-4">
                    ...
                    <div className={`${cardClass} overflow-hidden`}>
                        <table className="w-full text-sm">
                            <thead><tr className="border-b border-gray-100 dark:border-white/[0.06] text-left text-xs text-gray-500 uppercase">
                                <th className="px-4 py-3">Name</th><th className="px-4 py-3">Cost</th><th className="px-4 py-3">Drops</th><th className="px-4 py-3">Active</th>
                            </tr></thead>
                            <tbody>
                                {packs.length === 0 ? <tr><td colSpan={4}><Empty text="No packs. Seed defaults to create starter packs." /></td></tr> : packs.map((p: any) => (
                                    <tr key={p._id} className="border-b border-gray-50 dark:border-white/[0.03] hover:bg-gray-50/50 dark:hover:bg-white/[0.02]">
                                        <td className="px-4 py-3 font-medium">{p.name}</td>
                                        <td className="px-4 py-3 text-amber-600 font-bold">{p.pointsCost}</td>
                                        <td className="px-4 py-3">{p.drops?.length || 0} items</td>
                                        <td className="px-4 py-3">{p.isActive ? <span className="text-emerald-500">●</span> : <span className="text-gray-300">●</span>}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                    </div>
                    <p className="text-xs text-gray-400">Pack management (create/edit drops) coming soon. Use &quot;Seed Defaults&quot; in Overview to create starter packs.</p>
                </div>
            )}
                ...
                <div className="space-y-4">
                    ...
                    {configs.length === 0 ? (
                        <div className={`${cardClass} p-5 text-center`}>
                            <p className="text-sm text-gray-500 mb-3">No config found. Seed defaults first.</p>
                            <button onClick={handleSeed} disabled={seeding} className={btnPrimary}>{seeding ? 'Seeding...' : 'Seed Defaults'}</button>
                        </div>
                    ) : (
                        <div className={`${cardClass} divide-y divide-gray-100 dark:divide-white/[0.06]`}>
                            {configs.map((c: any) => (
                                <ConfigRow key={c._id} config={c} onSave={updateConfig} inputClass={inputClass} btnPrimary={btnPrimary} />
                            ))}
                        </div>
                    )}
                </div>
...
function OrdersPage() {
    ...
    const [editOrderForm, setEditOrderForm] = useState<{ items: { name: string; quantity: number; price: number }[]; totalPrice: number; status: string; paymentStatus: string }>({ items: [], totalPrice: 0, status: '', paymentStatus: '' })
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                {[
                    ...
                    <div key={i} className={`${cardClass} p-4 flex items-center gap-3`}>
                        ...
                        <div><p className={`text-lg font-bold ${s.color}`}>{s.value}</p><p className="text-[10px] text-gray-500 uppercase tracking-wider">{s.label}</p></div>
                    </div>
                ))}
            </div>
            ...
            <div className={`${cardClass} p-4 flex flex-wrap items-center gap-3`}>
                <div className="relative flex-1 min-w-[200px]">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                    <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by user, email, or order ID..." className="w-full pl-10 pr-4 py-2 rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-white/[0.04] text-sm focus:ring-2 focus:ring-primary-500/30 focus:border-primary-500 outline-none transition-all" />
                </div>
                <div className="flex gap-1.5">
                    {(['all', 'pending', 'completed', 'failed'] as const).map(f => (
                        <button key={f} onClick={() => setFilter(f)}
                            className={`px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all capitalize ${
                                filter === f ? 'bg-primary-600 text-white' : 'bg-gray-100 dark:bg-white/[0.06] text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-white/[0.1]'
                            }`}>
                            {f === 'all' ? `All (${stats.total})` : `${f} (${stats[f]})`}
                        </button>
                    ))}
                </div>
                <button onClick={loadOrders} className="p-2 rounded-lg bg-gray-100 dark:bg-white/[0.06] hover:bg-gray-200 dark:hover:bg-white/[0.1] transition-all">
                    ...
                    {filtered.map((order: any) => {
                        ...
                        return (
                            <div key={order._id} className={`${cardClass} overflow-hidden transition-all`}>
                                {/* Order header row */}
                                <div className="flex items-center gap-4 p-4 cursor-pointer hover:bg-gray-50/50 dark:hover:bg-white/[0.02] transition-all" onClick={() => setExpandedOrder(isExpanded ? null : order._id)}>
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-2 mb-1">
                                            <span className="text-xs font-mono text-gray-400 truncate">#{order._id.slice(-8)}</span>
                                            <span className={`px-2 py-0.5 rounded-lg text-[10px] font-bold uppercase ${statusColor[order.status] || ''}`}>{order.status}</span>
                                            <span className={`px-2 py-0.5 rounded-lg text-[10px] font-bold uppercase ${statusColor[order.paymentStatus] || 'bg-gray-100 dark:bg-white/[0.06] text-gray-500'}`}>{order.paymentStatus}</span>
                                        </div>
                                        <p className="text-sm font-medium text-gray-900 dark:text-white">
                                            {order.userId?.username || 'Unknown user'} <span className="text-gray-400 font-normal text-xs">{order.userId?.email}</span>
                                        </p>
                                    </div>
                                    <div className="text-right flex-shrink-0">
                                        <p className="text-lg font-bold text-gradient">${order.totalPrice?.toFixed(2)}</p>
                                        <p className="text-[10px] text-gray-400">{order.totalItems} items &middot; {new Date(order.createdAt).toLocaleDateString()}</p>
                                    </div>
                                        ...
                                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                                            <div className="border-t border-gray-100 dark:border-white/[0.06] p-4 space-y-4">
                                                ...
                                                {editingOrderId === order._id ? (
                                                    /* ── EDIT MODE ── */
                                                    <>
                                                        <p className="text-xs font-semibold text-primary-500 uppercase tracking-wider mb-2 flex items-center gap-1"><Edit3 className="w-3 h-3" /> Editing Order</p>
                                                        <div className="space-y-2">
                                                            {editOrderForm.items.map((item, idx) => (
                                                                <div key={idx} className="flex items-center gap-2 p-2 rounded-xl bg-gray-50 dark:bg-white/[0.02]">
                                                                    <input value={item.name} onChange={e => updateEditItem(idx, 'name', e.target.value)} className="flex-1 px-2 py-1 bg-white dark:bg-white/[0.04] border border-gray-200 dark:border-white/[0.08] rounded-lg text-sm focus:outline-none focus:border-primary-300 text-gray-700 dark:text-gray-300" placeholder="Name" />
                                                                    <input type="number" value={item.quantity} onChange={e => updateEditItem(idx, 'quantity', e.target.value)} className="w-16 px-2 py-1 bg-white dark:bg-white/[0.04] border border-gray-200 dark:border-white/[0.08] rounded-lg text-sm text-center focus:outline-none focus:border-primary-300 text-gray-700 dark:text-gray-300" placeholder="Qty" min={1} />
                                                                    <span className="text-gray-400 text-xs">×</span>
                                                                    <input type="number" step="0.01" value={item.price} onChange={e => updateEditItem(idx, 'price', e.target.value)} className="w-24 px-2 py-1 bg-white dark:bg-white/[0.04] border border-gray-200 dark:border-white/[0.08] rounded-lg text-sm text-center focus:outline-none focus:border-primary-300 text-gray-700 dark:text-gray-300" placeholder="Price" min={0} />
                                                                    <span className="text-sm font-semibold text-gray-500 w-20 text-right">${(item.price * item.quantity).toFixed(2)}</span>
                                                                </div>
                                                            ))}
                                                        </div>
                                                        <div className="flex items-center gap-3 flex-wrap">
                                                            <div>
                                                                <label className="text-[10px] text-gray-500 uppercase block mb-1">Status</label>
                                                                <select value={editOrderForm.status} onChange={e => setEditOrderForm({ ...editOrderForm, status: e.target.value })} className="px-3 py-1.5 bg-white dark:bg-white/[0.04] border border-gray-200 dark:border-white/[0.08] rounded-lg text-sm focus:outline-none focus:border-primary-300 text-gray-700 dark:text-gray-300">
                                                                    <option value="pending">Pending</option><option value="completed">Completed</option><option value="failed">Failed</option>
                                                                </select>
                                                            </div>
                                                            <div>
                                                                <label className="text-[10px] text-gray-500 uppercase block mb-1">Payment</label>
                                                                <select value={editOrderForm.paymentStatus} onChange={e => setEditOrderForm({ ...editOrderForm, paymentStatus: e.target.value })} className="px-3 py-1.5 bg-white dark:bg-white/[0.04] border border-gray-200 dark:border-white/[0.08] rounded-lg text-sm focus:outline-none focus:border-primary-300 text-gray-700 dark:text-gray-300">
                                                                    <option value="pending">Pending</option><option value="paid">Paid</option><option value="failed">Failed</option>
                                                                </select>
                                                            </div>
                                                            <div>
                                                                <label className="text-[10px] text-gray-500 uppercase block mb-1">Total</label>
                                                                <p className="text-lg font-bold text-gradient">${editOrderForm.totalPrice.toFixed(2)}</p>
                                                            </div>
                                                        </div>
                                                        <div className="flex items-center gap-2 pt-2 border-t border-gray-100 dark:border-white/[0.06]">
                                                            <button onClick={(e) => { e.stopPropagation(); handleOverride() }} disabled={isUpdating} className="px-4 py-1.5 rounded-lg text-xs font-semibold bg-primary-600 text-white hover:bg-primary-700 transition-all disabled:opacity-50 flex items-center gap-1">
                                                                {isUpdating ? <div className="w-3 h-3 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <Save className="w-3 h-3" />} Save Override
                                                            </button>
                                                            <button onClick={(e) => { e.stopPropagation(); setEditingOrderId(null) }} className="px-4 py-1.5 rounded-lg text-xs font-semibold bg-gray-100 text-gray-600 dark:bg-white/[0.06] dark:text-gray-400 hover:bg-gray-200 transition-all">Cancel</button>
                                                        </div>
                                                    </>
                                                ) : (
                                                    /* ── VIEW MODE ── */
                                                    <>
                                                        ...
                                                        <div>
                                                            ...
                                                            <div className="space-y-2">
                                                                {order.items.map((item: any, idx: number) => (
                                                                    <div key={idx} className="flex items-center gap-3 p-2 rounded-xl bg-gray-50 dark:bg-white/[0.02]">
                                                                        <div className="w-10 h-10 rounded-lg bg-gray-100 dark:bg-white/[0.04] overflow-hidden flex-shrink-0 flex items-center justify-center">
                                                                            {item.productId?.image ? <img src={item.productId.image} alt="" className="w-full h-full object-cover" /> : <Package className="w-4 h-4 text-gray-300" />}
                                                                        </div>
                                                                        <div className="flex-1 min-w-0">
                                                                            <p className="text-sm font-medium text-gray-900 dark:text-white truncate">{item.name}</p>
                                                                            <p className="text-[10px] text-gray-400">{item.category || 'N/A'} &middot; Qty: {item.quantity}</p>
                                                                        </div>
                                                        ...
                                                        {/* Actions */}
                                                        <div className="flex items-center gap-2 pt-2 border-t border-gray-100 dark:border-white/[0.06]">
                                                            <p className="text-xs text-gray-500 mr-auto">Created {new Date(order.createdAt).toLocaleString()}</p>
                                                            <button
                                                            ...
                                                            {order.status === 'pending' && (
                                                                <>
                                                                    <button
                                                                        onClick={(e) => { e.stopPropagation(); handleStatusChange(order._id, 'completed', 'paid') }}
                                                                        disabled={isUpdating}
                                                                    ...
                                                                    >
                                                                        {isUpdating ? '...' : '✓ Complete & Mark Paid'}
                                                                    </button>
                                                                    <button
                                                                        onClick={(e) => { e.stopPropagation(); handleStatusChange(order._id, 'failed', 'failed') }}
                                                                        disabled={isUpdating}
                                                            ...
                                                            {order.status === 'completed' && (
                                                                <span className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-emerald-100 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-400 flex items-center gap-1"><Check className="w-3 h-3" /> Order Fulfilled</span>
                                                            )}
                                                            {order.status === 'failed' && (
                                                                <button
                                                                    onClick={(e) => { e.stopPropagation(); handleStatusChange(order._id, 'pending', 'pending') }}
                                                                    disabled={isUpdating}
...
function ConfigRow({ config, onSave, inputClass, btnPrimary }: { config: any; onSave: (key: string, value: string) => void; inputClass: string; btnPrimary: string }) {
    ...
    return (
        <div className="flex items-center gap-4 px-5 py-4">
            ...
            <input value={value} onChange={e => { setValue(e.target.value); setDirty(true) }} className={`${inputClass} w-32 text-center`} />
            {dirty && <button onClick={() => { onSave(config.key, value); setDirty(false) }} className={btnPrimary}>Save</button>}
        </div>
    )
}
